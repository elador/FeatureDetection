...
void Renderer::setViewport(unsigned int screenWidth, unsigned int screenHeight)
{
...
cv::Mat Renderer::constructViewTransform()
{
...
cv::Mat Renderer::constructProjTransform()
{
...
void Renderer::setMesh(const Mesh* mesh) // make an optional argument, an index-list of vertices or so to render, if we don't want to render the full mesh.
{
	currentMesh = mesh;
}

void Renderer::setTransform(const cv::Mat transform)
{
	currentTransform = transform;
}

void Renderer::draw(ushort trianglesNum)
{
	DrawCall drawCall;
...
void Renderer::runVertexProcessor()
{
...
Vertex Renderer::runVertexShader(const Mesh* mesh, const cv::Mat& transform, const int vertexNum)
{
	Vertex output;
...
void Renderer::processProspectiveTriangleToRasterize(const Vertex& _v0, const Vertex& _v1, const Vertex& _v2, const Texture* _texture)
{
...
std::vector<Vertex> Renderer::clipPolygonToPlaneIn4D(const std::vector<Vertex>& vertices, const cv::Vec4f& planeNormal)
{
...
bool Renderer::areVerticesCCWInScreenSpace(const Vertex& v0, const Vertex& v1, const Vertex& v2)
{
...
cv::Vec3f Renderer::runPixelShader(const Texture* texture, const cv::Vec3f& color, const cv::Vec2f& texCoord, bool useTexturing)
{
	if(useTexturing) {
...
cv::Vec3f Renderer::tex2D(const Texture* texture, const cv::Vec2f& texCoord)
{
	return (1.0f / 255.0f) * tex2D_linear_mipmap_linear(texture, texCoord);
}

cv::Vec3f Renderer::tex2D_linear_mipmap_linear(const Texture* texture, const cv::Vec2f& texCoord)
{
...
cv::Vec2f Renderer::texCoord_wrap(const cv::Vec2f& texCoord)
{
	return cv::Vec2f(texCoord[0]-(int)texCoord[0], texCoord[1]-(int)texCoord[1]);
}

cv::Vec3f Renderer::tex2D_linear(const Texture* texture, const cv::Vec2f& imageTexCoord, unsigned char mipmapIndex)
{
...
float Renderer::clamp(float x, float a, float b)
{
	return std::max(std::min(x, b), a);
}

cv::Mat Renderer::getRendererImage()
{
	cv::Mat res;
...
cv::Mat Renderer::getRendererDepthBuffer()
{
