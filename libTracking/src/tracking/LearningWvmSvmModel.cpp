...
void LearningWvmSvmModel::evaluate(cv::Mat& image, std::vector<Sample>& samples) {
	delete fdImage;
	fdImage = new FdImage();
	fdImage->load(&image);
...
void LearningWvmSvmModel::update(cv::Mat& image, std::vector<Sample>& positiveSamples,
