...
int main(int argc, char *argv[])
{
	...
	// 1. Calculate the mean-shape of all training images
	...
	int numImages = labeledImageSource->getNames().size();
	int numModelLandmarks = 66; // Todo: Do dynamically?
	Mat groundtruthLandmarks(2 * numModelLandmarks, numImages, CV_32FC1);
	int currentImage = 0;
	while (labeledImageSource->next()) {
		vector<shared_ptr<Landmark>> lmsv = labeledImageSource->getLandmarks().getLandmarks();
	...
	Mat groundtruthLandmarksMean;
	cv::reduce(groundtruthLandmarks, groundtruthLandmarksMean, 1, CV_REDUCE_AVG); // reduce to 1 column

	// 2. For every training image:
	while (labeledImageSource->next()) {
		// a) Run the face-detector (the same that we're going to use in the testing-stage)
		...
		if (detectedFaces.empty()) {
			// no face detected. Skip the image in the whole training?
		}
		for (const auto& f : detectedFaces) {
			cv::rectangle(img, f, cv::Scalar(0.0f, 0.0f, 255.0f));
		}
		//detectedFaces[0] // tl_x, tl_y, w, h
		// b) Align the model? Rigid alignment? x_0
		// initialShape;
	...
	//	3. For every sample plus the original image: (loop through the matrix)
	...
	// 4. add one row to the features, add regLambda
	//		solve for x!

	// 5. Do the same again for all cascade steps
