	...
	for (auto& video : videoSigset) {
		...
		for (int frameNum = 0; frameNum < frames.size(); ++frameNum) {
			...
			if (tlx < 0 || tlx + w >= frames[frameNum].cols || tly < 0 || tly + h >= frames[frameNum].rows) {
				...
				appLogger.debug("Throwing away patch because it goes outside the image bounds.");
				continue;
			}
		...
		int minIdx, maxIdx;
		cv::minMaxIdx(scores, &minScore, &maxScore, &minIdx, &maxIdx);
		int idOfBestFrame = frameIds[maxIdx];
