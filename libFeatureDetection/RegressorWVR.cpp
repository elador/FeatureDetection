...
bool RegressorWVR::classify(FdPatch* fp)
{
	...
	if(fout_insert.second == false) {
		std::cout << "[RegressorWVR] An element 'fout' already exists for this detector. 'fout' not changed. You ran the same detector twice over a patch." << std::endl;
	}
...
int RegressorWVR::load(const char* filename)
{
	...
	if (pmxarray != 0) { // read area
		...
		if (pmxarray == 0) {
			printf("[RegrWVR]  Unable to find the matrix \'support_hk%d\'\n", 1);
			return 0;
		}
	...
	printf("[RegrWVR] Reading rectangles in area...\n");
