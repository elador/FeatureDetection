...
//	}
	...
	int v=1;
	if (!configReader->getInt("FD.roi.#0",&v))		std::cout << "[DetSVM] WARNING: Key in Config nicht gefunden, key:'FD.roi.#0', nehme Default: " << this->roiDistFromBorder.left << std::endl;
	else										this->roiDistFromBorder.left=v;
	if (!configReader->getInt("FD.roi.#1",&v))		std::cout << "[DetSVM] WARNING: Key in Config nicht gefunden, key:'FD.roi.#1', nehme Default: " << this->roiDistFromBorder.top << std::endl;
	else										this->roiDistFromBorder.top=v;
	if (!configReader->getInt("FD.roi.#2",&v))		std::cout << "[DetSVM] WARNING: Key in Config nicht gefunden, key:'FD.roi.#2', nehme Default: " << this->roiDistFromBorder.right << std::endl;
	else										this->roiDistFromBorder.right=v;
	if (!configReader->getInt("FD.roi.#3",&v))		std::cout << "[DetSVM] WARNING: Key in Config nicht gefunden, key:'FD.roi.#3', nehme Default: " << this->roiDistFromBorder.bottom << std::endl;
