...
//	}
	...
	if (!configReader->getInt("FD.roi.#1",&v))		std::cout << "[RegrSVR] WARNING: Key in Config nicht gefunden, key:'FD.roi.#1', nehme Default: " << this->roiDistFromBorder.top << std::endl;
	else										this->roiDistFromBorder.top=v;
	if (!configReader->getInt("FD.roi.#2",&v))		std::cout << "[RegrSVR] WARNING: Key in Config nicht gefunden, key:'FD.roi.#2', nehme Default: " << this->roiDistFromBorder.right << std::endl;
