...
int main(int argc, char *argv[])
{
	...
	*/
	...
	while(labeledImageSource->next()) {
		...
		ImageLoggers->getLogger("detection").setCurrentImageName(labeledImageSource->getName().stem().string() + "_" + "FaceFrontalFake");
		...
		vector<shared_ptr<imageprocessing::Patch>> llc;
		llc.push_back(make_shared<imageprocessing::Patch>(146, 153, 30, 30, cv::Mat())); // real
		llc.push_back(make_shared<imageprocessing::Patch>(133, 139, 30, 30, cv::Mat())); // FP, nose
		vector<shared_ptr<imageprocessing::Patch>> rlc;
		rlc.push_back(make_shared<imageprocessing::Patch>(111, 158, 30, 30, cv::Mat())); // real
		rlc.push_back(make_shared<imageprocessing::Patch>(111, 142, 30, 30, cv::Mat())); // fp
		vector<shared_ptr<imageprocessing::Patch>> lec;
		lec.push_back(make_shared<imageprocessing::Patch>(147, 109, 30, 30, cv::Mat()));
		//lec.push_back(make_shared<imageprocessing::Patch>(100, 100, 30, 30, cv::Mat()));
		vector<shared_ptr<imageprocessing::Patch>> rec;
		rec.push_back(make_shared<imageprocessing::Patch>(103, 114, 30, 30, cv::Mat()));
		//rec.push_back(make_shared<imageprocessing::Patch>(100, 100, 30, 30, cv::Mat()));
		vector<shared_ptr<imageprocessing::Patch>> nt;
		nt.push_back(make_shared<imageprocessing::Patch>(116, 133, 30, 30, cv::Mat()));
		//nt.push_back(make_shared<imageprocessing::Patch>(100, 100, 30, 30, cv::Mat()));
		...
		landmarkData2.insert(make_pair("left.lips.corner", llc));
		landmarkData2.insert(make_pair("right.lips.corner", rlc));
		landmarkData2.insert(make_pair("left.eye.pupil.center", lec));
		landmarkData2.insert(make_pair("right.eye.pupil.center", rec));
		landmarkData2.insert(make_pair("center.nose.tip", nt));

		Mat lmImg = img.clone();
		for (const auto& feature : landmarkData2) {
			appLogger.debug(feature.first + " #cand: " + lexical_cast<string>(feature.second.size()));
			for (const auto& lm : feature.second) {
				ModelLandmark ml(feature.first, lm->getX(), lm->getY());
				ml.draw(lmImg);
			}
		}
		...
		map<string, shared_ptr<imageprocessing::Patch>> resultLms = rnscnew.run(rnsacImg, 15.0f, 5); // It would somehow be helpful to have a LandmarkSet data-type, consisting of #n strings and each with #m Patches, and having delete, add, ... operations. Can we do this with only the STL? (probably)
		...
		vector<shared_ptr<ClassifiedPatch>> facePatches; // TMP
