...
ConstantVelocityModel::ConstantVelocityModel(double positionDeviation, double sizeDeviation) :
		positionDeviation(positionDeviation), sizeDeviation(sizeDeviation),
		generator(boost::mt19937(time(0)), boost::normal_distribution<>()) {}

void ConstantVelocityModel::init(const Mat& image) {}

void ConstantVelocityModel::predict(vector<shared_ptr<Sample>>& samples, const Mat& image, const shared_ptr<Sample> target) {
